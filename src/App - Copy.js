...
import {
  SYSTEM_TYPE_FLOORING, SYSTEM_TYPE_CEILINGS, SYSTEM_TYPE_PARTITIONS, SYSTEM_TYPE_WALL, SYSTEM_TYPE_ENVELOPES,SYSTEM_TYPE_INSULATION, DATASET_NAMES, materialListEnvelope, materialListFlooring, materialListCeilings, materialListPartitions,materialListInsulation, materialListWall,
  CHART_TYPES_ENVELOPES,
  TAB_INDEX_ENVELOPES, TAB_INDEX_FLOORING, TAB_INDEX_CEILINGS,TAB_INDEX_PARTITIONS,TAB_INDEX_WALL,TAB_INDEX_INSULATION, TAB_INDEX_OTHER
} from './CommonUtil';
...
// Insulation 没有 Material Breakdown，所以 ready 判定要跳过 materialData*
const DATASET_NAMES_INSULATION = DATASET_NAMES.filter(
  (n) => !n.startsWith("materialData")
);
...
class App extends Component {
  constructor(props) {
    ...
    DATASET_NAMES.forEach(dataSetName => {
      ...
      this.state[`insulation_${dataSetName}`] = [];
  ...
  handleTabChange = (event, newValue) => {
    ...
    if (this.state.insulation_selectedMaterials.length != 0) {
      this.setState({ value: newValue, insulation_selectedMaterials: this.state.insulation_selectedMaterials });
    } else {
      const DEFAULT_UNCHECKED = "bdXPSlegacy";

      this.setState({
        value: newValue,
        insulation_selectedMaterials: (this.state.insulation_materials || []).filter(
          m => m !== DEFAULT_UNCHECKED
        )
      });
    }
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (!_.isEqual(prevState, this.state)) {
      ...
      //
      // INSULATION
      let insulation_dataEnvelopes = {};
      let insulation_selectedDataEnvelopes = {};

      DATASET_NAMES_INSULATION.forEach(dataSetName => {
        let insulation_dataSetName = `insulation_${dataSetName}`;
        insulation_dataEnvelopes[dataSetName] = this.state[insulation_dataSetName] || [];

        if (this.state.insulation_selectedMaterials && this.state.insulation_selectedMaterials.length > 0) {
          insulation_selectedDataEnvelopes[dataSetName] =
            insulation_dataEnvelopes[dataSetName].filter(d => this.state.insulation_selectedMaterials.includes(d.material));
        } else {
          insulation_selectedDataEnvelopes[dataSetName] = [];
        }
      });

      // ready：只检查 Insulation 真的会用到的 datasets
      let insulation_dataEnvelopesReady = true;
      DATASET_NAMES_INSULATION.forEach(dataSetName => {
        insulation_dataEnvelopesReady =
          insulation_dataEnvelopesReady &&
          (insulation_dataEnvelopes[dataSetName] && insulation_dataEnvelopes[dataSetName].length > 0);
      });

      if (insulation_dataEnvelopesReady) {
        this.setState({
          insulation_dataEnvelopes,
          insulation_selectedDataEnvelopes,
          insulation_dataEnvelopesReady
        });
      }
      ...
      // --- URL params (ONLY apply "system" when it's actually in the URL) ---
      ...
      const hasSystemParam = mSystem != null

      if (type == null) type = 0
      if (mLenses == null) mLenses = "0_0_0"
      if (mChartType == null) mChartType = this.state.chartType
      if (mLifespan == null) mLifespan = this.state.lifespan
      if (mBiogenicCarbon == null) mBiogenicCarbon = this.state.biogenicCarbon

      // 不要在 system 缺失时强行生成 0_1_2_... 的默认串
      // 只在 URL 真有 system 的时候，才用它覆盖 state
      const baseState = {
        chartType: mChartType,
        lifespan: mLifespan,
        biogenicCarbon: mBiogenicCarbon
      }
      if (hasSystemParam) baseState.systemString = mSystem
      this.setState(baseState)

      // 只有 URL 带 system 时，才从 URL 覆盖“勾选的材料”
      if (hasSystemParam) {
        ...
        if (type == 0) { // envelope
          ...
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 0, systemString: mSystem, selectedMaterials: urlSelectedMaterials })

        } else if (type == 5) { // flooring
          let allSystems = materialListFlooring.map(m => m.value)
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 5, systemString: mSystem, flooring_selectedMaterials: urlSelectedMaterials })

        } else if (type == 4) { // ceilings
          let allSystems = materialListCeilings.map(m => m.value)
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 4, systemString: mSystem, ceilings_selectedMaterials: urlSelectedMaterials })

        } else if (type == 2) { // partitions
          let allSystems = materialListPartitions.map(m => m.value)
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 2, systemString: mSystem, partitions_selectedMaterials: urlSelectedMaterials })

        } else if (type == 3) { // wall
          let allSystems = materialListWall.map(m => m.value)
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 3, systemString: mSystem, wall_selectedMaterials: urlSelectedMaterials })

        } else if (type == 1) { // insulation
          let allSystems = materialListInsulation.map(m => m.value)
          let urlSelectedMaterials = names.map(n => allSystems[parseInt(n)])
          this.setState({ value: 1, systemString: mSystem, insulation_selectedMaterials: urlSelectedMaterials })
        }
      }
      // --- end URL params ---
  ...
  componentDidMount() {
      ...
      this.setState(
        {
          value: 1,
          systemString: mSystem,
          insulation_selectedMaterials: urlSelectedMaterials
        }
      )
    }
      ...
      LoadData[dataSetName](data => {
        if (idx === 0) {
          ...
          this.setState(prev => ({
            ...
            names,
            selectedMaterials:
              (prev.selectedMaterials && prev.selectedMaterials.length > 0)
      ...
      FlooringLoadData[dataSetName](data => {
        if (idx === 0) {
          ...
          this.setState(prev => ({
            ...
            flooring_selectedMaterials:
              (prev.flooring_selectedMaterials && prev.flooring_selectedMaterials.length > 0)
                ? prev.flooring_selectedMaterials
                : materials
          }));
      ...
      CeilingsLoadData[dataSetName](data => {
        if (idx === 0) {
          ...
          this.setState(prev => ({
            ...
            ceilings_selectedMaterials:
              (prev.ceilings_selectedMaterials && prev.ceilings_selectedMaterials.length > 0)
                ? prev.ceilings_selectedMaterials
                : materials
          }));
        } else {
      ...
      PartitionsLoadData[dataSetName](data => {
        if (idx === 0) {
          ...
          this.setState(prev => ({
            ...
            partitions_selectedMaterials:
              (prev.partitions_selectedMaterials && prev.partitions_selectedMaterials.length > 0)
                ? prev.partitions_selectedMaterials
                : materials
          }));
        } else {
      ...
      WallLoadData[dataSetName](data => {
        if (idx === 0) {
          ...
          this.setState(prev => ({
            ...
            wall_names: names,
            wall_selectedMaterials:
              (prev.wall_selectedMaterials && prev.wall_selectedMaterials.length > 0)
                ? prev.wall_selectedMaterials
                : materials
          }));          
        } else {
    ...
    DATASET_NAMES_INSULATION.forEach((dataSetName, idx) => {
      InsulationLoadData[dataSetName](data => {
        if (idx === 0) {
          const materials = data.map(d => d.material);
          const names = data.map(d => d.name);
          const DEFAULT_UNCHECKED = "bdXPSlegacy";

          const defaultSelected = materials.filter(m => m !== DEFAULT_UNCHECKED);

          this.setState(prev => ({
            [`insulation_${dataSetName}`]: data,
            insulation_materials: materials,
            insulation_names: names,
            insulation_selectedMaterials:
              (prev.insulation_selectedMaterials && prev.insulation_selectedMaterials.length > 0)
                ? prev.insulation_selectedMaterials
                : defaultSelected
          }));

        } else {
          this.setState({ [`insulation_${dataSetName}`]: data });
        }
      });
    });
  ...
  async constructURL() {
    ...
    } else if (this.state.value == 1) { // insulation
      let allSystems = materialListInsulation.map(m => m.value);
      for (let i = 0; i < allSystems.length; i++) {
        if (this.state.insulation_selectedMaterials.includes(allSystems[i])) {
          selectedString += i + "_";
        }
      }
    }
  ...
  updateSelectedInsulationMaterials(newSelectedMaterials) {
    let selectedString = "";
    let allSystems = materialListInsulation.map(m => m.value);
    ...
    this.setState({ insulation_selectedMaterials: newSelectedMaterials });
  ...
  render() {
    ...
    if (this.state.insulation_dataEnvelopesReady) {
      for (let i = 0; i < this.state.insulation_selectedDataEnvelopes.gwpData.length; i++) {
        let myName = this.state.insulation_selectedDataEnvelopes.gwpData[i].material;
        let myVal = this.state.insulation_selectedDataEnvelopes.gwpData[i].value;
        insulation_obj[myName] = myVal;
      }
    }
    ...
    return (
      <div className="App" style={{ minHeight: sidebarHeight }}>
        ...
        {/* INSULATION */}
        <TabPanel className={styles.tabPanel} value={this.state.value} index={TAB_INDEX_INSULATION} style={{marginLeft:'5%', marginRight:'5%'} }>
          ...
          <div id="parentDiv">
            ...
            {this.state.insulation_materials.length > 0 &&
              <div className={styles.sidebar} id="sidebar123">
                <MaterialList
                  units={unitsForType(SYSTEM_TYPE_INSULATION)}
                  type={SYSTEM_TYPE_INSULATION}
                  title={"Insulation Type"}
                  hasMaterialHealth={true}
                  gwp={insulation_obj}
                  materials={this.state.insulation_materials}
                  names={this.state.insulation_names}
                  updateSelectedMaterials={this.updateSelectedInsulationMaterials.bind(this)}
                  initialSelectedMaterials={this.state.insulation_selectedMaterials}
                  metaData={InsulationLoadData.metaData}
                  currentSel={this.state.chartType}
                  matBreakdown={this.state.insulation_materialData1}
                  matBreakdown1={this.state.insulations_materialData3}
                  matBreakdown2={this.state.insulation_materialData5}
                  tenYGWP={this.state.insulation_gwpData1}
                  sixty1YGWP={this.state.insulation_gwpData2}
                  sixty2YGWP={this.state.insulation_gwpData5}
                />
              </div>
            }
            <div className={styles.chartContainer}>
              <h1>{chartTitle}</h1>
              {this.state.insulation_dataEnvelopesReady && (
                <ChartContainer
                  units={unitsForType(SYSTEM_TYPE_INSULATION)}
                  type={SYSTEM_TYPE_INSULATION}
                  chartType={this.state.chartType}
                  ...
                  biogenicCarbon={this.state.biogenicCarbon}
                  metaData={InsulationLoadData.metaData}
                  data={this.state.insulation_dataEnvelopes || {}}
                  selectedData={this.state.insulation_selectedDataEnvelopes || {}}
                  ready={this.state.insulation_dataEnvelopesReady === true}
                />
              )}
                  ...
                  {this.state.rows.map((row, idx) => {
                    return (
                      <Row
                        ...
                        name={idx + 1}
                        count={0}
                        tenY={this.state.insulation_gwpData1}
                        sixty1={this.state.insulation_gwpData2}
                        sixty2={this.state.insulation_gwpData5}
                        radio={this.state.currentRadio}
                        divStyle={divStyle}
                        onChange={(e) => this.updateValue(e, idx)}
                        onChecked={() => this.onChecked(idx)}
          ...
          <div id="parentDiv">
            ...
            {this.state.partitions_materials.length > 0 &&
              <div className={styles.sidebar} id="sidebar123">
                ...
                <MaterialList
                  units={unitsForType(SYSTEM_TYPE_PARTITIONS)}
